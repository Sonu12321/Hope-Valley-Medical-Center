...
const Chatbot = () => {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '60px', right: '20px', border: '1px solid #ccc', borderRadius: '10px', padding: '10px', width: '300px', backgroundColor: '#565945' }}>
      <div style={{ height: '300px', overflowY: 'scroll', marginBottom: '10px' }}>
        {messages.map((msg, index) => (
          <div key={index} style={{ textAlign: msg.user === 'user' ? 'right' : 'left', padding: '5px 0' }}>
            <p style={{ backgroundColor: msg.user === 'user' ? '#698474' : '#FFD3B6', padding: '10px', borderRadius: '10px' }}>{msg.text}</p>
          </div>
        ))}
      </div>
      <input 
